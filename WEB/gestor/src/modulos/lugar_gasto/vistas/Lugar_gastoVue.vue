    ...
    <section class="contenedor_section_tabla">
        ...
        <div v-else v-for="(lugar_gasto, index) in lugar_gastos" :key="index" class="bloque-transaccion">
            <div class="bloque-detalle" id="tablaTransaccion">
                ...
                <p><strong>Name: </strong>{{ lugar_gasto.nombre }}</p>
            </div>
...
<script setup lang="ts">
    ...
    const { traeLugar_gasto, lugar_gastos } = useLugar_gasto();

    // Cuando la página es visible y está cargada
    onMounted(async () => {
        await traeLugar_gasto();  // Llamar a la función que obtiene los datos
    });

    const lugar_gastoPDF = async () => {
        const pagina = document.getElementById('tablaLugar_gasto'); // Asegúrate que el ID sea correcto
        if (!pagina) {
            console.error("Elemento 'tablaLugar_gasto' no encontrado.");
            return;
        }
