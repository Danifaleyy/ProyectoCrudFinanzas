...
<template>
    ...
    <section class="contenedorinicio">
        <div class="contenedor_botones">
            ...
            <RouterLink :to = "{path: '/portafolio/agregar'}">
            ...
            <button class="btn btn-sm boton_accion" v-if="portafolios.length > 0">
                <download-excel :data="portafolios" type="xlsx" name="portafolio.xlsx">
    ...
    <section class="contenedor_section_tabla">
        ...
        <div v-if="portafolios.length == 0" class="no-registros">
            Sin portafolio registrado
        </div>

        <!-- Mostrar las transacciones como bloques -->
        <div v-else v-for="(portafolio, index) in portafolios" :key="index" class="bloque-transaccion">
            <div class="bloque-detalle" id="tablaIngreso">
                <!--<p><strong>ID:</strong> {{ ingreso.id_ingreso }}</p>-->
                <p><strong>Instrument: </strong> {{ portafolio.fk_id_instrumento }}</p>
                <p><strong>Investment Type: </strong> {{ portafolio.fk_id_tipo_inversion }}</p>
                <p><strong>Description: </strong> {{ portafolio.descripcion }}</p>
                <p><strong>Amount: </strong> {{ portafolio.monto }}</p>
                <p><strong>Rate: </strong> {{ portafolio.tasa }}</p>
                <p><strong>Start Date: </strong> {{ dayjs(portafolio.fecha_inicio).format('DD-MM-YYYY') }}</p>
                <p><strong>End Date:</strong> {{ dayjs(portafolio.fecha_final).format('DD-MM-YYYY') }}</p>
                <!--<p><strong>comprobante: </strong> {{ portafolio.comprobante }}</p>-->
            </div>

            <div class="bloque-acciones">
                    <button type="button" class="boton_accion_botonmodificar">
                        <!--Boton modificar-->
                        <RouterLink class="nav-link item" :to = "{path: '/portafolio/'+ portafolio.id_portafolio + '/editar'}"><i class="fa fa-pencil"></i></RouterLink>
                    </button>
                    <button type="button" class="boton_accion_botonborrar">
                        <!--Boton eliminar-->
                        <RouterLink class="nav-link item" :to = "{path: '/portafolio/'+ portafolio.id_portafolio + '/borrar'}"><i class="ri-delete-bin-fill"></i></RouterLink>
                    </button>
            </div>
        </div>
    </section>
...
<script setup lang="ts">
    ...
    const { traePortafolio, portafolios} = usePortafolio();
    //Cuando la página es visible y está cargada
    onMounted(async () => {
        await traePortafolio();
    });

    const portafolioPDF = async () => {
        const pagina = document.getElementById('tablaPortafolio');
        if (!pagina) {
            console.error("Elemento 'tablaPortafolio' no encontrado.");
            return;
        }
