<template>
    <h3>Investment Instrument</h3>
    <section class="contenedor_section">
        <div class="contenedor_botones">
            <!-- Botón para agregar un nuevo registro -->
            <RouterLink :to="{ path: '/instrumento/agregar' }">
            ...
            <button class="btn btn-sm boton_accion" v-if="instrumentos.length > 0">
                <download-excel :data="instrumentos" type="xlsx" name="instrumento.xlsx">
    ...
    <section class="contenedor_section_tabla">
        <!-- Si no hay instrumentos, mostrar un mensaje -->
        <div v-if="instrumentos.length == 0" class="no-registros">
            Sin instrumento registrado
        </div>

        <!-- Mostrar los instrumentos en bloques -->
        <div v-else v-for="(instrumento, index) in instrumentos" :key="index" class="bloque-transaccion">
            <div class="bloque-detalle" id="tablaInstrumento">
                <!--<p><strong>ID:</strong> {{ instrumento.id_instrumento }}</p>-->
                <p><strong>Instrument: </strong> {{ instrumento.nombre }}</p>
            </div>
            <div class="bloque-acciones">
                <!-- Botón para editar -->
                <button type="button" class="boton_accion_botonmodificar">
                    <RouterLink :to="{ path: '/instrumento/' + instrumento.id_instrumento + '/editar' }"><i class="fa fa-pencil"></i></RouterLink>
                </button>
                <!-- Botón para borrar -->
                <button type="button" class="boton_accion_botonborrar">
                    <RouterLink :to="{ path: '/instrumento/' + instrumento.id_instrumento + '/borrar' }"><i class="fa fa-trash"></i></RouterLink>
                </button>
            </div>
        </div>
    </section>
...
<script setup lang="ts">
    ...
    const { traeInstrumento, instrumentos } = useInstrumento();

    // Cuando la página es visible y está cargada
    onMounted(async () => {
        await traeInstrumento();
    });

    const instrumentoPDF = async () => {
        const pagina = document.getElementById('tablaInstrumento');
        if (!pagina) {
            console.error("Elemento 'tablaInstrumento' no encontrado.");
            return;
        }
