<template>
    ...
    <section class="contenedorinicio">
        <div class="botones">
            ...
            <RouterLink :to = "{path: '/cuenta/agregar'}">
            ...
            &nbsp;
            <!--Boton para imprimir-->
            <button @click.prevent="cuentaPDF" class="btn btn-sm btn-outline-primary" v-if="cuentas.length > 0">
                Imprimir <i class="fa fa-print"></i>
            </button>
            &nbsp;
            <!--Agregamos un boton nuevo para Excel-->
            <button class="btn btn-sm btn-outline-primary" v-if="cuentas.length > 0">
                <download-excel :data="cuentas" type="xlsx" name="cuenta.xlsx">
    ...
    <section class="ContenedorTabla">
        <table class="desingtable" id="tablaCuenta">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr v-if="cuentas.length == 0">
                    <td class="centrado" colspan="3">Sin cuenta registrada</td>
                </tr>
                <!--Ciclo for para que muestre tantos como sean-->
                <tr v-else v-for="(cuenta, index) in cuentas" :key="index">
                    ...
                    <td class="centrado">
                        <div class="btn-group" role="group" aria-label="Basic outlined example">
                            <button type="button" class="btn btn-sm btn-outline-primary botonmodificar">
                                <!--Boton modificar-->
                                <RouterLink class="nav-link item" :to = "{path: '/cuenta/'+ cuenta.id_cuenta + '/editar'}"><i class="fa fa-pencil"></i></RouterLink>
                            </button>
                            <button type="button" class="btn btn-sm btn-outline-danger botonborrar">
                                <!--Boton eliminar-->
                                <RouterLink class="nav-link item" :to = "{path: '/cuenta/'+ cuenta.id_cuenta + '/borrar'}"><i class="fa fa-trash"></i></RouterLink>
                            </button>
                        </div>
                    </td>
                </tr>
...
<script setup lang="ts">
    ...
    const { traeCuenta, cuentas } = useCuenta();
    ...
    const cuentaPDF = async () => {
        const pagina = document.getElementById('tablaCuenta');
        if (!pagina) {
            console.error("Elemento 'tablaCuenta' no encontrado.");
            return;
        }
